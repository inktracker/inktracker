...
const FLOOR_TASKS = {
  ...
  // "Confirm delivery date" was dropped — the supplier returns it on the
  // order and operators don't need a separate gate.
...
export default function Production() {
  ...
  async function floorToggleGoods(order, liIdx, size) {
    try {
      const checklist = { ...(order.checklist || {}) };
      ...
      if (!next) return;
      gp[key] = { status: next, by: user?.full_name || user?.email || "Admin", at: new Date().toISOString() };
  ...
  // Soft-warn wrapper — override allowed for partial-ship scenarios.
  function floorAdvanceWithGoodsGuard(order, nextStatus) {
    if (order?.status === "Order Goods" && nextStatus === "Pre-Press") {
      ...
      if (missing > 0) {
        const ok = window.confirm(`${missing} of ${total} sizes haven't been marked received.\n\nMove to Pre-Press anyway?`);
        if (!ok) return;
      }
    }
    return floorUpdateStatus(order, nextStatus);
  }
              ...
              <div className="flex-1 overflow-y-auto" style={{ maxHeight: "70vh" }}>
                {!sel ? (
                  ...
                  <div className="p-5 space-y-4 max-w-2xl mx-auto">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          {nextStep && (
                            <button onClick={() => floorAdvanceWithGoodsGuard(sel, nextStep)} disabled={floorUpdating}
                    ...
                    {/* Checklist — Order Goods auto-derives Place blank
                        order + Receive goods from goods_progress. */}
                    {(() => {
                      ...
                      const counts = countGoodsProgress(sel);
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                          ...
                          <div className="space-y-1">
                            {tasks.map(task => {
                              ...
                              return (
                                <button key={task}
                                  onClick={() => { if (auto === null) floorToggleTask(sel, task); }}
                                  disabled={auto !== null}
                                  title={auto !== null ? "Auto-tracked from per-size status below" : undefined}
                                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition ${done ? "bg-emerald-50 border border-emerald-200" : "bg-white dark:bg-slate-900 hover:bg-slate-100 border border-transparent"} ${auto !== null ? "cursor-default" : ""}`}>
                    ...
                    {/* Job ticket — stage-aware.
                        Order Goods: ordered (API) → received (manual) per size.
                        Printing:    per-imprint print tracking.
                        Other:       read-only quantity. */}
                    {(() => {
                      const stage = sel.status || "Pre-Press";
                      const goodsProgress = sel.checklist?.goods_progress || {};
                      const printProgress = sel.checklist?.print_progress || {};
                      const { total: goodsTotal, ordered: goodsOrdered, received: goodsReceived } = countGoodsProgress(sel);
                      return (
                        <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                          <div className="flex items-center justify-between mb-3">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {sortSizeEntries(Object.entries(li.sizes || {})).filter(([, v]) => parseInt(v) > 0).map(([size, count]) => {
                                      ...
                                      if (stage === "Order Goods") {
                                        ...
                                        return (
                                          <button key={size}
                                            onClick={() => floorToggleGoods(sel, idx, size)}
                                            disabled={!actionable}
                                            title={
                                      ...
                                      return (
                                        <span key={size}
                                          className="text-sm rounded-xl px-4 py-2.5 font-bold border-2 bg-white dark:bg-slate-900 border-slate-200 text-slate-700">
                                          {size}: {count}
                                        </span>
                                      );
                                    })}
