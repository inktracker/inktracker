...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    // Guards against two search batches racing: the debounce cleanup only
    // clears a PENDING timer, not an already-fired request. If a slow batch
    // resolves after a newer one, it must not overwrite the fresher results.
    let cancelled = false;
                  ...
                  {results.customers.length > 0 && (
                    <div className="p-4">
                      ...
                      {results.customers.map(c => (
                        <a key={c.id} href={`${createPageUrl("Customers")}?open=${c.id}`} onClick={() => setOpen(false)} className="block px-3 py-2 rounded hover:bg-slate-50 text-sm">
                  ...
                  {results.orders.length > 0 && (
                    <div className="p-4">
                      ...
                      {results.orders.map(o => (
                        <a key={o.id} href={`${createPageUrl("Orders")}?id=${o.id}`} onClick={() => setOpen(false)} className="block px-3 py-2 rounded hover:bg-slate-50 text-sm">
                  ...
                  {results.quotes.length > 0 && (
                    <div className="p-4">
                      ...
                      {results.quotes.map(q => (
                        <a key={q.id} href={`${createPageUrl("Quotes")}?id=${q.id}`} onClick={() => setOpen(false)} className="block px-3 py-2 rounded hover:bg-slate-50 text-sm">
                  ...
                  {results.invoices.length > 0 && (
                    <div className="p-4">
                      ...
                      {results.invoices.map(i => (
                        <a key={i.id} href={`${createPageUrl("Invoices")}?id=${i.id}`} onClick={() => setOpen(false)} className="block px-3 py-2 rounded hover:bg-slate-50 text-sm">
