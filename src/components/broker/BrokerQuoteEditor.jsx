...
function withBrokerDefaults(quote) {
  return {
    ...
    tax_rate: 0,
    broker_tax_rate: quote?.broker_tax_rate ?? 0,
...
export default function BrokerQuoteEditor({
  ...
  // Client-facing totals use the broker's chosen tax rate (defaults to 0).
  // Tax-exempt customers always get 0 regardless of what the broker entered,
  ...
  async function runSave(status) {
    ...
    setSaving(true);
    try {
      // A saved quote is a contract — every total the shop and the broker
      // and the client see comes from THIS save's stamped values. No
      // viewer recomputes after this point. We stamp BOTH sides:
      //   _ppp / _lineTotal / _rushFee         → broker-side (BROKER_MARKUP)
      //   _client_ppp / _client_lineTotal / …  → client-side (STANDARD_MARKUP)
      // clientPpp override applies ONLY to the client-side stamping
      // (broker negotiating a discount with their end client). The
      // broker-side stamp is the shop's wholesale rate, never overridden.
      ...
      const stampedItems = (q.line_items || []).map(li => {
        ...
        const brokerPpp      = brokerR ? brokerR.ppp : 0;
        const brokerLineTot  = brokerPpp * qty;
        const brokerRushFee  = brokerR ? brokerR.rushFee : 0;
        ...
        return {
          ...li,
          _ppp:               brokerPpp,
          _lineTotal:         brokerLineTot,
          _rushFee:           brokerRushFee,
          _client_ppp:        clientPpp,
          _client_lineTotal:  clientLineTot,
          _client_rushFee:    clientRushFee,
        };
      });
      ...
      const clientLineSub = stampedItems.reduce((s, li) => s + (li._client_lineTotal || 0), 0);
      const clientRushSum = stampedItems.reduce((s, li) => s + (li._client_rushFee || 0), 0);
      ...
      await onSave({
        ...
        tax: 0,
        total: brokerTotal,
        // Client side — what broker dashboard + client PDF read.
        client_subtotal: clientSub,
        client_tax: clientTaxAmt,
        client_total: clientTotalAmt,
  ...
  // Submit-to-shop is no longer a save-time action. It happens from the
  // broker dashboard drawer, and only after the client has approved the
  // quote. See QuoteDetailDrawer.canSubmitToShop in BrokerDashboard.jsx.
    ...
    <ModalBackdrop onClose={onClose} z="z-50" dismissOnBackdropClick={false}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-wrap gap-3 px-6 py-4 bg-slate-50 rounded-b-2xl">
          ...
          {/* "Submit to Shop" lives on the broker dashboard drawer now, gated
              on Client Approved status. Quotes must go to the client first;
              the shop only sees them after the client signs off. */}

          <button
            onClick={() => previewPdf(exportQuoteToPDF(
              {
                ...q,
                broker_id: broker?.email || "broker",
                // Stamp the broker profile onto the quote so the shop form
                // shows full broker contact details (company, name, email,
                // phone) regardless of what was on the quote at submit time.
                broker_company: q?.broker_company || broker?.company_name || "",
                broker_name: q?.broker_name || broker?.full_name || broker?.display_name || "",
                broker_email: q?.broker_email || broker?.email || "",
                broker_phone: q?.broker_phone || broker?.broker_phone || "",
              },
              {
                mode: "shop",
                shopName: shop?.shop_name || shop?.owner_email || "",
            ...
            title="Preview full internal shop form"
          >
