...
function ArtworkPreviewOverlay({ art, onClose }) {
  ...
  // the order modal's containing block (its outer wrapper has
  // `backdrop-blur-sm` → a CSS filter → which creates a new containing
  // block and was confining `fixed inset-0` inside the modal instead of
    ...
    >
      ...
      <div className="flex-1 bg-slate-100 flex items-center justify-center overflow-auto min-h-0">
        {isImage ? (
          ...
          // <object> with <iframe> fallback renders PDFs more
          // consistently across browsers than a bare iframe — Chrome
          // honors the viewer fragment, Safari doesn't crop the page.
