...
function Stage({
  ...
  // ── inline/landing additions ──────────────────────────────────────────
  // The demos were built to run one-per-iframe as a design tool. Mounted
  // inline on the marketing page these three defaults are actively harmful,
  // so each is opt-out:
  //   persist  — localStorage playhead makes a demo resume mid-scene
  //   keyboard — a window-level Space handler preventDefaults page scroll
  ...
  React.useEffect(() => {
    if (playingProp == null) return;
    setPlaying(playingProp);
  }, [playingProp]);

  // Replay: any change to seekNonce rewinds to t=0.
...
export { Easing, clamp, interpolate, animate, useTime, useTimeline, useSprite, Sprite, TextSprite, ImageSprite, RectSprite, Stage };
