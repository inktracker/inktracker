...
AS $$
  SELECT jsonb_build_object(
    ...
    'payments_24h_count', (SELECT count(*)              FROM public.invoices WHERE paid > 0 AND paid_date >= now() - interval '1 day'),
    'payments_24h_total', (SELECT coalesce(sum(paid),0)  FROM public.invoices WHERE paid > 0 AND paid_date >= now() - interval '1 day'),
    'payments_7d_count',  (SELECT count(*)              FROM public.invoices WHERE paid > 0 AND paid_date >= now() - interval '7 days'),
    'payments_7d_total',  (SELECT coalesce(sum(paid),0)  FROM public.invoices WHERE paid > 0 AND paid_date >= now() - interval '7 days')
  );
$$;
