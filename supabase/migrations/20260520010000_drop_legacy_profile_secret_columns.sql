...
-- 2 & 3. Backfill from profiles → profile_secrets. The backfills only make
-- sense BEFORE the legacy columns are dropped from `profiles`. On a database
-- where this migration already ran (and the columns are already gone), the
-- backfills are no-ops by definition. Skip the whole block in that case so
-- we don't error on missing columns when re-running through `db push`.
DO $$
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'ss_account_number'
  ) THEN
    -- 2a. Backfill ss_account_number — prefer profiles (where Account.jsx
    -- wrote) over the legacy profile_secrets.ss_account.
    ...
    -- 3. Catch-all backfill: any other secret column still on profiles.
