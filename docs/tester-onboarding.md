...
4. **Connect QuickBooks** under **Account → Integrations → QuickBooks**. OAuth flow opens in a new tab. After approving, you'll land back in InkTracker with the green "Connected" badge. Then run through the **[QB setup checklist](qb-shop-setup-checklist.md)** (attached) — a few settings inside QBO itself (Payments activation, sales tax, customer emails) decide whether invoices and payment links behave the way you expect.
